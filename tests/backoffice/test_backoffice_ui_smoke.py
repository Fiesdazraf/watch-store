...
# Only enable when DEBUG=True and ENABLE_DJDT=True (default True in dev)
ENABLE_DJDT = DEBUG and (os.getenv("ENABLE_DJDT", "True") == "True")
if ENABLE_DJDT:
...
# Put DebugToolbar middleware at the very beginning (only when enabled)
if ENABLE_DJDT:
...
# Internal IPs + toolbar show policy (only when enabled)
if ENABLE_DJDT:
    INTERNAL_IPS = ["127.0.0.1", "localhost"]
    # If you want to hide the bar temporarily without uninstalling:
    # DJDT_SHOW=False → hides the panel even though app/middleware loaded
    _djdt_show = os.getenv("DJDT_SHOW", "True") == "True"
    DEBUG_TOOLBAR_CONFIG = {
        "SHOW_TOOLBAR_CALLBACK": (lambda request: _djdt_show),
    }
